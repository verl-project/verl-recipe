...
VAL_FILES="${VAL_FILES:-}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${RECIPE_ROOT}/outputs/qwen3moe_sft}"
PROJECT_NAME="${PROJECT_NAME:-verlbb-qwen3moe-sft}"
...
if [[ "${PAD_MODE}" != "no_padding" ]]; then
  echo "VerlBB currently supports PAD_MODE=no_padding only." >&2
  exit 1
fi
...
RUN_NAME="${RUN_NAME:-qwen3moe_sft_${BACKEND}_tp${TP_SIZE}_pp${PP_SIZE}_cp${CP_SIZE}_ep${EP_SIZE}_etp${ETP_SIZE}}"
...
if [[ "${BACKEND}" == "megatron" ]]; then
  BACKEND_ARGS=(
    ...
    "+engine.override_transformer_config.attention_backend=${ATTENTION_BACKEND}"
  )
  if [[ "${OPTIMIZER_OFFLOAD}" == "True" || "${OPTIMIZER_OFFLOAD}" == "true" || "${OPTIMIZER_OFFLOAD}" == "1" ]]; then
    BACKEND_ARGS+=(
      "+optim.override_optimizer_config.optimizer_offload_fraction=${OPTIMIZER_STATE_OFFLOAD_FRACTION}"
      ...
      "+optim.override_optimizer_config.optimizer_cpu_offload=${OPTIMIZER_CPU_OFFLOAD}"
