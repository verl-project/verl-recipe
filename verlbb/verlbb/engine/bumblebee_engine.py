"""External VERL engine backed by Bumblebee runtime primitives."""
...
from bumblebee.model import resolve_model_type_from_hf
from bumblebee.primitive.ckpt import load_training_checkpoint, save_training_checkpoint
from bumblebee.primitive.parallel import pack_nested_thd, unpack_packed_thd_to_nested
from bumblebee.primitive.protocols import default_expert_classifier, default_placement_fn
from bumblebee.runtime import create_runtime
from bumblebee.runtime.backends.bb.config import BBConfig
from bumblebee.runtime.contracts.config import (
    OptimizerConfig as BBOptimizerConfig,
)
from bumblebee.runtime.contracts.config import (
    ParallelConfig,
    RuntimeConfig,
...
from .config import BumblebeeEngineConfig
...
def _build_lr_scheduler(optimizer, opt: BBOptimizerConfig):
    """Build a Megatron-style LR scheduler for Bumblebee's optimizer."""
...
class _BumblebeeModeCtx(BaseEngineCtx):
    """Wrap Bumblebee runtime contexts with VERL's offload behavior."""

    def __init__(self, engine: BumblebeeEngine, mode: str, **kwargs):
...
@EngineRegistry.register(model_type="language_model", backend="bumblebee", device="cuda")
class BumblebeeEngine(BaseEngine):
    """VERL BaseEngine implementation that delegates model lifecycle to Bumblebee."""

    def __init__(
        self,
        model_config: HFModelConfig,
        engine_config: BumblebeeEngineConfig,
    ...
    def initialize(self):
        ...
        self._bb_config = self._build_bb_config()
        self.runtime = create_runtime(
            RuntimeConfig(
                backend="bb",
                hf_path=self.model_config.local_path,
                backend_cfg=self._bb_config,
            )
        )
        ...
        if self.handle._optimizer is not None and self.handle._lr_scheduler is None:
            self.handle._lr_scheduler = _build_lr_scheduler(self.handle._optimizer, self._bb_config.optimizer)
    ...
    def train_mode(self, **kwargs):
        ...
        return _BumblebeeModeCtx(self, mode="train", **kwargs)

    def eval_mode(self, **kwargs):
        self._require_initialized()
        return _BumblebeeModeCtx(self, mode="eval", **kwargs)
    ...
    def forward_backward_batch(self, data: TensorDict, loss_function, forward_only: bool = False) -> dict[str, Any]:
        ...
        if pad_mode != DatasetPadMode.NO_PADDING:
            raise NotImplementedError("BumblebeeEngine only supports pad_mode=no_padding for now.")
    ...
    def save_checkpoint(
        ...
        **kwargs,
        ...
        if save_contents is not None and not any(item in save_contents for item in ("model", "optimizer")):
            if self._rank == 0:
                print(f"Skipping Bumblebee checkpoint save at step {global_step}: save_contents={save_contents}")
    ...
    def _require_initialized(self) -> None:
        if self.runtime is None or self.handle is None:
            raise RuntimeError("BumblebeeEngine is not initialized yet.")

    def _build_bb_config(self) -> BBConfig:
        return BBConfig(
            ...
            optimizer=self._build_bb_optimizer_config(),
    ...
    def _build_impl_cfg(self) -> dict[str, Any]:
        ...
        if impl_cfg.get("use_thd", True) is not True:
            raise ValueError("BumblebeeEngine supports only THD/no-padding SFT; set engine.impl_cfg.use_thd=True.")
    ...
    def _build_bb_optimizer_config(self) -> BBOptimizerConfig:
    ...
    def _normalize_optimizer_name(config: OptimizerConfig) -> str:
        ...
        if "adam" in lower:
            return "adam"
        raise ValueError(f"BumblebeeEngine only supports Adam-style optimizers today, got {optimizer_name!r}")

    def _extract_primary_module(self):
        model = self.handle._model
        if isinstance(model, list | tuple):
            if not model:
                raise RuntimeError("Bumblebee runtime returned an empty model chunk list.")
            if len(model) > 1:
    ...
    def _forward_backward_batch_with_runtime(
        ...
        *,
        ...
        if batch_num_tokens is None:
            raise ValueError("BumblebeeEngine PP/CP SFT requires batch_num_tokens for VERL-compatible loss scaling.")
    ...
    def _make_model_inputs(self, micro_batch: TensorDict) -> dict[str, torch.Tensor]:
        ...
        if not getattr(input_ids, "is_nested", False):
            raise NotImplementedError("BumblebeeEngine supports only nested no-padding THD batches.")
    ...
    def _build_verl_model_output(
        ...
        *,
        ...
        if log_probs is None:
            raise ValueError("Bumblebee THD model output must contain token log_probs.")
    ...
    def _scalar_temperature(micro_batch: TensorDict) -> float:
        ...
        if not torch.all(values.detach() == first).item():
            raise NotImplementedError("BumblebeeEngine currently supports scalar temperature only.")
