...
async def compute_reward(
    ...
    has_content = [hc and (not is_gd) for hc, is_gd in zip(has_content, gd_mask, strict=True)]
    non_empty = [g for g, keep in zip(generations, has_content, strict=True) if keep]
        ...
        for metric, config in metrics.items():
            ...
            if metric == "state_reward_on_response":
                ...
                if config_path:
                    ...
                    sub_config = json.load(open(config_path))
    ...
    for metric, config in metrics.items():
        ...
        if hasattr(module, "compute_batch_score"):
            ...
            assert isinstance(scores_and_info, list), (
                f"Expected list from compute_batch_score, got {type(scores_and_info)}"
            )
        ...
        for key in out_keys:
            ...
            for keep in has_content:
                full_scores_and_info.append(
                    (scores_and_info[idx][key] if key is not None else scores_and_info[idx]) if keep else auto_field
                )
                idx += keep
...
class HumanLMRewardManager(RewardManagerBase):
    ...
    def __init__(self, config, tokenizer, compute_score=None, **kwargs):
        ...
        self._train_metrics_cfg = reward_kwargs.get(
            "train_metrics", {"response": {"state_reward": {"weight": 1.0, "kwargs": {}}}}
        )
        ...
        if state_config:
            self.state_config = json.loads(open(state_config).read())
    ...
    async def _compute_rewards_async(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        if len(self.state_config) == 1:
            ...
            extra_info = [{**info, "state_desc": self.state_config[info["state_name"]]["desc"]} for info in extra_info]
        ...
        prompt_ids_no_pad = [ids[mask.bool()].tolist() for ids, mask in zip(prompt_ids, attention_mask, strict=True)]
        ...
        for i, (g, h) in enumerate(zip(raw_generations, state_names, strict=True)):
        ...
        should_have_field_masks = {
            field: torch.tensor(should_have_field_masks[field], dtype=torch.bool) for field in should_have_field_masks
        }
        print(
        ...
        generation_fields_by_keys = {
            key: [generation_fields[i] for i in range(batch_size) if keys[i] == key] for key in unique_keys
        }
        field_to_metrics_by_keys = {key: field_to_metrics_lst[i] for i, key in enumerate(keys)}
        ...
        for key in generation_fields_by_keys:
            ...
            for field in field_to_metrics:
                metrics_to_run = {m: v for m, v in field_to_metrics[field].items()}
        ...
        print("[compute_reward] gather complete", flush=True)
        ...
        if self.split == "val" and self.eval_push_to_hub:
            for i, (key, prompt, gen, raw_gen, gt, extra) in enumerate(
                zip(keys, prompts, generation_fields, raw_generations, ground_truth, extra_info, strict=True)
            ):
        ...
        if self.separate_generation:
            ...
            try:
                    ...
                    for metric in self.field_to_metrics[field]:
                        ...
                        for sub_metric in sorted(sub_metrics):
                            ...
                            scores_by_fm[fm_key] = torch.tensor(
                                [reward_dicts[i][field][sub_metric] for i in range(batch_size)]
                            )
                ...
                raise ValueError("[Error] field not found in reward output") from err

            # Apply field-metric specific weights
            weighted_scores_by_fm = {fm: scores_by_fm[fm] * self.field_metric_weights[fm] for fm in scores_by_fm}

        # Combine weighted scores from all field and metrics into a single tensor
        if scores_by_fm:
            scores = torch.stack([weighted_scores_by_fm[fm] for fm in scores_by_fm]).sum(dim=0)
        else:
            scores = torch.zeros(batch_size)
        ...
        if scores_by_fm:
            log_weighted_scores_by_field_metric = {
                f"{self.split}/{fm}": weighted_scores_by_fm[fm][should_have_field_masks[fm.split(":")[0]]]
                .mean(dim=0)
                .item()
        ...
        log_weighted_scores_by_field_metric.update(
            {
                **{
                    f"{self.split}/valid_rate/{h if self.separate_generation else 'all'}": torch.tensor(valid_rate)
                    .mean()
                    .item()
                    for h, valid_rate in valid_mask_by_state.items()
                },
        ...
        print(
            f"[DEBUG METRICS] log_weighted_scores_by_field_metric: {log_weighted_scores_by_field_metric}",
            flush=True,
        )
...
if "humanlm" not in REWARD_MANAGER_REGISTRY:
    register("humanlm")(HumanLMRewardManager)
else:
