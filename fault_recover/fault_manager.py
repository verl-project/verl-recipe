...
class TokensDict:
    ...
    def clear(self, latest_model_ckpt_step):
        with self._lock:
            while not all([req_info.get("finished", False) for _, req_info in self.index_prompt_tokens.items()]):
                time.sleep(1)
...
class ChipStatus:
    def __init__(self, world_size):
        self.free = [False] * world_size
        self.check = True

    def is_free(self, index: int = None):
        if index is not None:
            return self.free[index]
        return all(self.free)

    def set_free(self, status: bool, index: int):
        self.free[index] = status
...
class FaultMgr:
    ...
    def bind_trainer(cls, trainer):
        ...
        AgentLoopManager.generate_sequences = cls.catch_rollout_fault(
            cls.timeout(AgentLoopManager.generate_sequences),
            roles=[Role.ActorRollout, Role.RefPolicy]
        )
                ...
                if config.fault_manager.enable:
                    ...
                    while (max_reschedule_times > 0) and (reschedule_times < max_reschedule_times):
                        try:
                            ...
                            print(
                                f"[fault_manager][{datetime.datetime.now()}] catch reschedule fault: {e} during recover, reschedule_times: {reschedule_times}/{max_reschedule_times}")
                            reschedule_error = e
                            reschedule_times += 1
                        else:
                            break
    ...
    def rebuild_wg(cls, roles: list):
        if not cls.trainer:
            raise ValueError(f'[fault_manager] Have not bind trainer!')
            ...
            try:
                ...
                while (max_rebuild_times < 0) or (rebuild_times < max_rebuild_times):
                    try:
                        ...
                        gen_batch_output = cls._update_gen_batch(gen_batch_output,
                                                                 ray.get(cls.tokens_dict.get.remote()))
                        ...
                        print(
                            f"[fault_manager][{datetime.datetime.now()}] catch rebuild fault: {e} during recover retry, rebuild_times: {rebuild_times}/{max_rebuild_times}")
                        rebuild_error = e
                        rebuild_times += 1
                raise rebuild_error
    ...
    def timeout(cls, func):
        ...
        def wrapper(_self, prompts):
            ...
            if not _self.config.fault_manager.enable or timeout_task_check_interval < 0 or prompts.meta_info.get(
                    'validate'):
    ...
    def _get_wg_kwargs(cls):
        ...
        if OmegaConf.select(cls.trainer.config.global_profiler, "steps") is not None:
            ...
            if OmegaConf.select(cls.trainer.config.global_profiler, "tool") == "nsys":
                assert (
                        OmegaConf.select(cls.trainer.config.global_profiler.global_tool_config.nsys,
    ...
    def _parse_req_tokens(cls, req_info, td):
        for req_id, new_token_info in req_info.items():
            if req_id in cls.request_global_id_map:
                ray.get(
                    td.extend.remote(cls.request_global_id_map[req_id], "new_token_ids",
                                     new_token_info['new_tokens_ids']))
                ray.get(td.set.remote(cls.request_global_id_map[req_id], "finished", new_token_info['finished']))
